...
window.KHReports=(()=>{
 ...
 function load(src,global){if(window[global])return Promise.resolve(window[global]);if(pending.has(src))return pending.get(src);const task=new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=src;s.onload=()=>resolve(window[global]);s.onerror=()=>{pending.delete(src);s.remove();reject(Error('export library unavailable'))};document.head.appendChild(s)});pending.set(src,task);return task}
 function content(){
  const box=document.querySelector('#v07-brief-content');
  const rows=[...box.querySelectorAll('.v07-brief-item')].map(item=>({title:item.querySelector(':scope>div>strong')?.textContent||'',summary:item.querySelector(':scope>div>p')?.textContent||'',action:[...item.querySelectorAll('.v11-brief-decision>div')].map(x=>x.textContent.trim()),evidence:[...item.querySelectorAll('.kh-number-details dl>div')].map(x=>x.textContent.trim())}));
  return {title:box.querySelector('h1').textContent,subtitle:box.querySelector('header p').textContent,source:L(snapshotMode==='synthetic'?'بيانات تركيبية للتجربة':'بيانات جهة مستوردة — معالجة محلية',snapshotMode==='synthetic'?'Synthetic evaluation data':'Imported entity data — local processing'),summary:[...box.querySelectorAll('.v07-brief-summary>div')].map(x=>x.textContent.trim()),rows,fingerprint:stableStateFingerprint(),policy:KHPilot.revision(),unit:KHPlain.unitHelp()};
 }
 ...
 async function exportFile(kind){
  ...
  try{
   ...
   if(kind==='doc'){
    ...
    const para=(text,heading)=>new d.Paragraph({bidirectional:ar,alignment:ar?d.AlignmentType.RIGHT:d.AlignmentType.LEFT,heading,spacing:{after:140,line:320},children:[new d.TextRun({text,rtl:ar,font:'Arial',size:heading?30:23})]});
    ...
    data.rows.forEach((r,i)=>children.push(para(`${i+1}. ${r.title}`,d.HeadingLevel.HEADING_1),para(r.summary),...r.action.map(x=>para(x)),para(L('تفاصيل الدليل','Evidence details'),d.HeadingLevel.HEADING_2),...r.evidence.map(x=>para(x))));
    ...
    const P=await load('vendor/pptxgenjs-4.0.1.js?v=180','pptxgen'),ppt=new P();ppt.layout='LAYOUT_WIDE';ppt.author='Kinetic HR';ppt.subject=data.source;ppt.title=data.title;ppt.lang=ar?'ar-SA':'en-US';ppt.rtlMode=ar;
