...
function renderLandingExample(){
  ...
  box.innerHTML=`<div><span>${L('الفجوة الحالية','Current gap')}</span><strong>${fmt(deficit(r),1)} FTE</strong></div><div><span>${L('التمويل','Funding')}</span><strong>${f.known?`${fmt(f.funded,1)} / ${fmt(f.unfunded,1)}`:'—'}</strong></div><div><span>${L('أثر الخدمة','Service impact')}</span><strong>${imp?`${fmt(imp.value,0)} ${esc(imp.unit)}`:'—'}</strong></div><div><span>${L('قدرة داخلية قابلة للنقل','Transfer-ready capacity')}</span><strong>${transfer?fmt(transfer,1)+' FTE':'—'}</strong></div>`;
}
...
function ensureScenarioStructure(){
  ...
  if(controls&&compare&&compare.parentElement!==controls)controls.appendChild(compare);
...
function renderScenarioViz(){
  ...
  const max=Math.max(1,...base,...gaps)*1.12,w=620,h=240,p=30;
  ...
  box.innerHTML=`<div class="v12-chart-card"><div class="v12-chart-head"><div><strong>${L('أثر التدخل المتوقع على الفجوة','Expected intervention effect on the gap')}</strong><small>${L('المسار الأخضر يوضح السيناريو الحالي، والخط المتقطع يمثل عدم التدخل.','Green shows the current scenario; dotted gray represents no intervention.')}</small></div><div class="v12-chart-legend"><span><i></i>${L('مع التدخل','With action')}</span><span><i class="base"></i>${L('بدون تدخل','No action')}</span></div></div><div class="v12-chart-svg"><svg viewBox="0 0 ${w} ${h}" role="img"><g stroke="#edf2f5" stroke-width="1">${[0,1,2,3].map(i=>{const y=p+i*(h-2*p)/3;return `<line x1="${p}" x2="${w-p}" y1="${y}" y2="${y}"/>`}).join('')}</g><path d="${area}" fill="rgba(13,179,154,.10)"/><path d="${bpath}" fill="none" stroke="#aab8c4" stroke-width="2.4" stroke-dasharray="6 6"/><path d="${path}" fill="none" stroke="#0db39a" stroke-width="3.4" stroke-linecap="round" stroke-linejoin="round"/>${gaps.map((v,i)=>{const x=p+i*(w-2*p)/(samples-1),y=h-p-(v/max)*(h-2*p);return `<circle cx="${x}" cy="${y}" r="4.5" fill="#0db39a"/><text x="${x}" y="${h-7}" text-anchor="middle" font-size="9" fill="#7b8d99">${Math.round(horizon*i/(samples-1))}d</text>`}).join('')}</svg></div></div><div class="v12-outcome-stack"><article class="v12-outcome"><span>${L('الفجوة الحالية','Current gap')}</span><strong>${fmt(impact.beforeGap,1)} FTE</strong><small>${L('قبل التدخل','before action')}</small></article><article class="v12-outcome good"><span>${L('الفجوة عند نهاية الأفق','Gap at horizon')}</span><strong>${fmt(impact.afterGap,1)} FTE</strong><small>${L('بعد السيناريو','after scenario')}</small></article><article class="v12-outcome"><span>${L('أولوية القرار','Decision priority')}</span><strong>${priority??'—'} → ${impact.afterPriority??'—'}</strong><small>${L('أثر القرار على الأولوية','priority effect')}</small></article><article class="v12-outcome good"><span>${L('أثر الخدمة المتبقي','Remaining service impact')}</span><strong>${impAfter?`${fmt(impAfter.value,0)} ${esc(impAfter.unit)}`:'—'}</strong><small>${impBefore&&impAfter?L(`من ${fmt(impBefore.value,0)} قبل التدخل`,`from ${fmt(impBefore.value,0)} before action`):L('أدخل بيانات أثر الخدمة','service impact input required')}</small></article></div>`;
}
function renderScenarioKpis(){
  ...
  <article class="v12-skpi"><div class="v12-skpi-icon">◉</div><div><span>${L('الفجوة الحالية','Current gap')}</span><strong>${fmt(deficit(r),1)} FTE</strong><small>${esc(occupationLabel(r.ssco))}</small></div></article>
  <article class="v12-skpi"><div class="v12-skpi-icon">▤</div><div><span>${L('ممولة / غير ممولة','Funded / unfunded')}</span><strong>${f.known?`${fmt(f.funded,1)} / ${fmt(f.unfunded,1)}`:'—'}</strong><small>${L('FTE من الفجوة الحالية','FTE of current gap')}</small></div></article>
